...
function getTone(priority: ActionCenterItem["priority"]): ActionCenterMetric["tone"] {
...
export async function getActionCenterData(
  ...
  const trialBookedStale = leads.filter((lead) => lead.stage === "trial_booked" && !lead.nextFollowUpAt);
  const overduePayments = payments.filter((payment) => payment.status === "overdue");
  const dueTodayPayments = payments.filter((payment) => payment.status === "pending" && payment.dueDate.slice(0, 10) === new Date().toISOString().slice(0, 10));
  ...
  const mediumPriority: ActionCenterItem[] = [
    ...
    ...leadsWithoutFollowUp.map((lead) => ({
      ...
      meta: lead.nextFollowUpAt ? formatDate(lead.nextFollowUpAt, locale) : t(locale, "غير محدد", "Not scheduled"),
    })),
  ...
  const metrics: ActionCenterMetric[] = [
    {
